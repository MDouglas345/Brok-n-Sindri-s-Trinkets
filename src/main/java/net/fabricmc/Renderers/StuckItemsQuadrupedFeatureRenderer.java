...
public class StuckItemsQuadrupedFeatureRenderer<T extends LivingEntity, M extends QuadrupedEntityModel<T>> extends FeatureRenderer<T, M> {

    private final EntityRenderDispatcher dispatcher;
    private final ItemRenderer           itemRenderer;
    
    public StuckItemsQuadrupedFeatureRenderer(EntityRendererFactory.Context context, LivingEntityRenderer<?, ?> entityRenderer) {
    ...
    public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T livingEntity, float f, float g, float h, float j, float k, float l) {
        ...
        */
        ...
        Vec3d S = this.getPointOnPart(new Vec3d(random.nextFloat(), MathHelper.clamp(random.nextFloat(), 0.0f, 0.3f), -side), cuboid);
